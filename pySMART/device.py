...
class Device(object):
    ...
    """
    def __init__(self, name, interface=None, megaraid_number=None):
        ...
        self.megaraid_number = megaraid_number
        """ MegaRaid device number in RAID """
        ...
        **(dict of str):** Contains parsed and processed diagnostic information
        ...
        # If no interface type was provided, scan for the device
            ...
            #if device is a raid disk
            if self.megaraid_number is not None:
                cmd = Popen('smartctl --scan-open | {0} "{1}" | {0} "{2}"'.format(
                    _grep, self.name, self.megaraid_number), shell=True, stdout=PIPE, stderr=PIPE)
            else:
